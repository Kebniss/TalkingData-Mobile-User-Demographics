'''This model scores 2.26 on Kaggle'''
...
# it creates a new column with those rows of phone that have the same device id
# of train and test
gatrain['brand'] = phone['brand']
gatest['brand'] = phone['brand']
# create sparse matrix with one row for each train/test device id and one column
# per brand
...
# concatenate phone_brand with device_model in a single feature and encode it
...
def score(clf, nclasses, random_state = 0):
...
pred.to_csv(os.path.join(FEATURES_DATA_DIR,'logreg_subm.csv'),index=True)
